...
%
% March 2018, Modified by Ryan Elliott
...
% Plot eigenvalues
mask = (abs(l) > 1e-4 & imag(l) > pi*3e-2 & real(l) > -15);
uu = u(:,mask);
ll = l(mask);
%uu = u; 
%ll = l; 
% figure; plot(real(ll), imag(ll)/2/pi, 'rx');

% compass(real(uu(1:6,22)),imag(uu(1:6,22)));
% Mode shape analysis (generator states)
% States:
%    1 - angle
%    2 - speed
%    8 - V_As (exciter)
...
WECC_SHAPE_PLOT = 0;
if ( WECC_SHAPE_PLOT )
    genList = {'NW BC', 'NE BC', 'SE BC', 'NW WA', 'SW WA', 'NW OR', 'N WA', ...
               'SE WA', 'E WA', 'S WA', 'N OR', 'N ID', 'NW MT', 'E MT', 'SW OR', ...
               'N CA', 'NW CA', 'C CA', 'C CA', 'C CA', 'S CA', 'S CA', 'S CA', ...
               'S NV', 'S NV', 'S AZ', 'S AZ', 'NE AZ', 'C UT', 'E CO', 'SW WY', ...
               'C ID', 'N NV', 'C AB'};

    idx_state = find(mac_state(:,2)==9);
    idx_mode = 35;  % Frequency regulation: 1; Inter-area: 22, 36, 38, 39; Mixed K1,K2: 35
    u_vec = uu(idx_state,idx_mode);
    [u_sort,idx_sort] = sort(abs(u_vec), 'descend');
    u_vec = u_vec/max(abs(u_vec));
    u_vec = u_vec*exp(-1j*angle(u_vec(idx_sort(1))));

    % run('../data/plot_options');

    n_show = 34;
    cmap = lines(n_show);
    h = compass(real(u_vec(idx_sort(1:n_show))),imag(u_vec(idx_sort(1:n_show))));
    for ii = 1:n_show
        set(h(ii), 'color', cmap(ii,:), 'lineWidth', 3, 'lineStyle', '-.');
    end
    legend(h,genList{idx_sort(1:n_show)},'location','northeastOutside');

    idx_sort(1:n_show)

    print(gcf, '../data/mini_figures/common_mode_shape.eps', '-depsc2', '-r400');
end

KRK_SHAPE_PLOT = 0; 
if ( KRK_SHAPE_PLOT )
    idx_state = find(mac_state(:,2)==2);
    idx_mode = 2;
    
    if (idx_mode == 1)
        fig_str = 'mode_shape_freq';
    else
        fig_str = 'mode_shape_inter'; 
    end
    ...
    print(gcf, ['../data/krk_figures/', fig_str, '.eps'], '-depsc2', '-r400');
    
    my_shape_out = zeros(n_show,3); 
    for jj = 1:n_show
        my_shape_out(jj,:) = [jj, angle(u_vec(jj))*180/pi, abs(u_vec(jj))];
    end
    
    common_file = ['../data/krk_figures/', fig_str, '.dat'];
    common_fid = fopen(common_file, 'w'); 
    fprintf(common_fid,'k,ang,mag\n');
    dlmwrite(common_file,my_shape_out,'-append');
end
...
% clear a_mat acc ang_idx
% clear b_lmod b_pm b_pr b_rlmod b_vr basdat bus bus_sol
% clear c_ang c_Efd c_p c_pm c_spd c_t c_v
% clear d_angpr d_angvr d_ppr d_pvr d_vpr d_vvr damp dcr_dc
% clear freq full_dfile iter_max
% clear k_cpss k_p k_pss k_s_idx
% clear l lfpf lfs line m_a m_b M_h mac_state NumStates
% clear my_A my_B my_Kg my_Kp my_M my_Rg my_Ts my_Tw
% clear p p_norm R_g s_T4 svc_dc
% clear tcsc_dc tol Ts u v
