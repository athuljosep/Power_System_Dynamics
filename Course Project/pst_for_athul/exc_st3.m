...
if n_st3~=0
  ...
  if flag == 1 % network interface computation
    if i~=0 %exciter-by-exciter calculation
      ...
      vex(n,k) = Efd(i,k); %set field voltage for machines
        ...
        E_Isat(n) = max(eqprime(n,k),E_Isat(n)); % select higher voltage
                                        
        if ~isempty(nst3_sub) %check for any sub synchronous machine model
        % 
        %  REPLACED vector calculations 02/09/2018 Ryan Elliott
        %
            n_mac_number = n(nst3_sub);
          
            psidpp = mac_pot(n_mac_number,9).*eqprime(n_mac_number,k) ...
                   + mac_pot(n_mac_number,10).*psikd(n_mac_number,k);
            psiqpp = mac_pot(n_mac_number,14).*edprime(n_mac_number,k) ...
                   + mac_pot(n_mac_number,15).*psikq(n_mac_number,k);

            fldcur(n_mac_number,k) = E_Isat(n_mac_number) ...
                   + mac_pot(n_mac_number,6).*(eqprime(n_mac_number,k) - psikd(n_mac_number,k)) ...
                   + mac_pot(n_mac_number,7).*curdg(n_mac_number,k);

            ed(n_mac_number,k) = - mac_con(n_mac_number,5).*curdg(n_mac_number,k) ...
                   - (psiqpp - mac_con(n_mac_number,13).*curqg(n_mac_number,k));
            eq(n_mac_number,k) = - mac_con(n_mac_number,5).*curqg(n_mac_number,k) ...
                   + (psidpp - mac_con(n_mac_number,8).*curdg(n_mac_number,k));
        end
        ...
        %
            ...
            fldcur(n_mac_number,k) = E_Isat(n_mac_number) ...
                   + (mac_con(n_mac_number,6) - mac_con(n_mac_number,7)).*curdg(n_mac_number,k);
            ed(n_mac_number,k) = edprime(n_mac_number,k) ...
                   + mac_con(n_mac_number,7).*curqg(n_mac_number,k);
            eq(n_mac_number,k) = eqprime(n_mac_number,k) ...
                   - mac_con(n_mac_number,7).*curdg(n_mac_number,k);
        end
        ...
        ve = vep+jay*(exc_con(st3_idx,15)...
           + (exc_pot(st3_idx,1)+jay*exc_pot(st3_idx,2)).*exc_con(st3_idx,16)).*iterm;
        V_E = abs(ve); % this is the equivalent terminal voltage at the rectifier terminals
        ve_low = find(V_E < 1e-6);
  ...
  % For iagnostics only
  % [V_TR(:,k), eterm(:,k), V_A(:,k), Efd(:,k), V_R(:,k), V_B(:,k)]
  % [ed(:,k), edc(:,k), eq(:,k), eqc(:,k), curdg(:,k), curqg(:,k), eterm(:,k), V_B(:,k)]
