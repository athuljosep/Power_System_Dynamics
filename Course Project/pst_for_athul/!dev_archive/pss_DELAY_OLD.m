...
my_delay = ceil(my_dt/my_dt)*my_dt;  % desired delay
my_dsamp = my_delay/my_dt + 1;  % number of delayed samples
my_dbuff = ones(1,my_dsamp);  % shift register for delayed signal
   ...
   if flag == 1 % network interface computation
      if i ~= 0 % scalar computation
         ...
         elseif ( pss_con(i,1) == 2 )
            ...
            if ( numel(my_dbuff) > 1 )
                my_dbuff(2:end) = my_dbuff(1:end-1);
            end
            my_dbuff(1) = mean(mac_spd(wa_set, k));
            
            var1 = (my_Kwa*(mac_spd(i,k) - my_dbuff(end))...
                  + my_Kcm*(my_dbuff(end)-1) - pss1(i,k))/pss_con(i,4); 
         end
         if n_dpw~=0
            if n_dpw ~= 0
               i_dpw = find(dpw_pss_idx==i);
         ...
         if n_pss~=0
            ...
            if ~isempty(pss_wa_idx)  % wide-area PSS
                ...
                var1(pss_wa_idx) = (my_Kwa*(mac_spd(n_wa,k) - my_dbuff(end))...
                   + my_Kcm*(my_dbuff(end)-1) - pss1(n_wa,k))./pss_con(pss_wa_idx,4); 
            end
            if ~isempty(pss_p_idx)
                n_p = mac_int(pss_con(pss_p_idx,2));
      ...
      if i ~= 0 % scalar computation
         ...
         if n_pss~=0
            ...
            if ~isempty(pss_wa_idx)  % wide-area PSS
                ...
                my_dbuff(1) = mean(mac_spd(wa_set, k));
                var1(pss_wa_idx) = (my_Kwa*(mac_spd(n_wa,k) - my_dbuff(end))...
                   + my_Kcm*(my_dbuff(1)-1) - pss1(n_wa,k))./pss_con(pss_wa_idx,4);
            end
            if ~isempty(pss_p_idx)
                n_p = mac_int(pss_con(pss_p_idx,2));
