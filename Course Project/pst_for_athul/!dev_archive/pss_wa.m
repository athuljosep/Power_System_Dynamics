   ...
   if flag == 1 % network interface computation
      if i ~= 0 % scalar computation
         ...
         elseif ( pss_con(i,1) == 2 )
            ...
            var1 = (mac_spd(i,k)-K_wa*mean(mac_spd(wa_set, k))...
                +(K_wa-1)-pss1(i,k))/pss_con(i,4); 
         end
         if n_dpw~=0
            if n_dpw ~= 0
               i_dpw = find(dpw_pss_idx==i);
         ...
         if n_pss~=0
            ...
            if ~isempty(pss_wa_idx)  % wide-area PSS
               n_wa = mac_int(pss_con(pss_wa_idx,2));
               var1(pss_wa_idx) = (mac_spd(n_wa,k)-K_wa*mean(mac_spd(wa_set, k))...
                   +(K_wa-1)-pss1(n_wa,k))./pss_con(pss_wa_idx,4); 
            end
            if ~isempty(pss_p_idx)
               n_p = mac_int(pss_con(pss_p_idx,2));
