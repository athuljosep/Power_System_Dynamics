...
if n_pss~=0
   ...
   if flag == 1 % network interface computation
      if i ~= 0 % scalar computation
         n = pss_mb_idx(i); % machine number
         if pss_con(i,1) == 1
            var1 = (mac_spd(i,k)-K_wa*mean(mac_spd(wa_set, k))+(K_wa-1)-pss1(i,k))/pss_con(i,4);
         else
         ...
         if n_pss~=0
            ...
            if length(pss_sp_idx)~=0
               n_sp = mac_int(pss_con(pss_sp_idx,2));
               var1(pss_sp_idx) = (mac_spd(n_sp,k)-K_wa*mean(mac_spd(wa_set, k))+(K_wa-1)-pss1(n_sp,k))./pss_con(pss_sp_idx,4);
            end
            if ~isempty(pss_p_idx)
               n_p = mac_int(pss_con(pss_p_idx,2));
   ...
   if flag == 2 % pss dynamics calculation
      if i ~= 0 % scalar computation
         ...
         if n_pss~=0
            ...
            if ~isempty(pss_sp_idx)
               n_sp = mac_int(pss_con(pss_sp_idx,2));
               var1(pss_sp_idx) = (mac_spd(n_sp,k)-K_wa*mean(mac_spd(wa_set, k))+(K_wa-1)-pss1(n_sp,k))...
