...
if n_sub~=0
  if flag == 0; % initialization
      ...
      ei = v + (mac_con(mac_sub_idx,5)+jay*mac_con(mac_sub_idx,6)).*curr; % voltage behind sub-transient reactance in system frame
      ...
      rot = -jay*jay*exp(-jay*mac_ang(mac_sub_idx,1)); % system reference frame rotation to Park's frame
...
%       end
      ...
      ps = jay*(psidpp+jay*psiqpp).*exp(jay*mac_ang(mac_sub_idx,1));
      psi_re(mac_sub_idx,1) = real(ps);
      psi_im(mac_sub_idx,1) = imag(ps);
      clear ps
...
%                               sin(mac_ang(mac_sub_idx,1)).*psidpp; % imag part of psi
  ...
  if flag == 1 % network interface computation 
     % vectorized computation
      ...
      ps = jay*mac_spd(mac_sub_idx,k).*(psidpp+jay*psiqpp).*exp(jay*mac_ang(mac_sub_idx,k));
      psi_re(mac_sub_idx,k) = real(ps);
      psi_im(mac_sub_idx,k) = imag(ps);
      clear ps
...
%                               sin(mac_ang(mac_sub_idx,k)).*psidpp; % imag part of psi   
  ...
  if flag == 2 | flag == 3 % generator dynamics calculation
    % vectorized computation
      ...
      psidpp = mac_pot(mac_sub_idx,9).*eqprime(mac_sub_idx,k) + ...
               mac_pot(mac_sub_idx,10).*psikd(mac_sub_idx,k);
      c = -jay*(cur_re(mac_sub_idx,k)+jay*cur_im(mac_sub_idx,k)).*exp(-jay*mac_ang(mac_sub_idx,k)).*jay;
      curd(mac_sub_idx,k) = real(c);
      curq(mac_sub_idx,k) = imag(c);
      clear c
...
%       Te = pelect(mac_sub_idx,k).*mac_pot(mac_sub_idx,1) + mac_con(mac_sub_idx,5).*mcurmag.*mcurmag;
        Te = psidpp.*curq(mac_sub_idx,k) - psiqpp.*curd(mac_sub_idx,k);
