...
% THIS VERSION DISABLES THE VOLTAGE FEEDBACK; SEE LINES MARKED WITH XXXXXXXX
...
if ~isempty(svc_con)
   if flag == 0; % initialization
      if i~=0
         ...
         svc_pot(:,4) = B_cv(:,1)./svc_con(:,6);% reference voltage  XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
   ...
   if flag == 2 % exciter dynamics calculation
      ...
      if i ~= 0
         ...
         dB_cv(i,k) = (-B_cv(i,k)+svc_con(i,6)*(err*svc_pot(i,5)+B_con(i,k)))/svc_con(i,7);
            ...
            err =  svc_sig(:,k) + svc_pot(:,4) + d_sigin; %XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
