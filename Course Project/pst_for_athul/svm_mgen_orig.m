...
% Plot eigenvalues
ll = l(abs(l) > 1e-3 & imag(l) > 1e-3 & real(l) > -30);
figure; plot(real(ll), imag(ll)/2/pi, 'rx');

% State indices
ang_idx = find(mac_state(:,2) == 1);
spd_idx = find(mac_state(:,2) == 2);
exc_idx = find(mac_state(:,2) == 10);
pss_idx = find(mac_state(:,2) == 12);
gov_idx = find(mac_state(:,2) == 21);

plt_idx = spd_idx;
mode_num = 48;

fig1 = figure;
u_tmp = u(plt_idx, mode_num);
[max_val, max_idx] = max(abs(u_tmp));
rel_u = u(plt_idx ,mode_num)/max_val;
rel_u = rel_u*exp(-1i*angle(rel_u(max_idx)));
[rel_u, ord_u] = sort(rel_u, 'descend');
h = compass(rel_u);
...
cmap = lines(numel(h));
[~, map_idx] = sort(cmap(:,1), 'descend');
cmap = cmap(map_idx, :);

for ii = 1:length(h)
    h_xdata = get(h(ii), 'xdata');
    h_ydata = get(h(ii), 'ydata');

    set(h(ii), 'xdata', h_xdata(:), 'ydata', h_ydata(:));
    set(h(ii), 'color', cmap(ii,:), 'lineWidth', 4, 'lineStyle', '-.');
end

%gen_leg = {'Speed 1', 'Speed 2', 'Speed 3', 'Speed 4'};
gen_leg = {'Gov 1', 'Gov 2', 'Gov 3', 'Gov 4'};
legend(h(ord_p), gen_leg, 'Location', 'BestOutside');

print(fig1, '../data/figures/gov_mode_shape', '-depsc2', '-r400');
print(fig1, '../data/figures/gov_mode_shape', '-dpng', '-r400');
