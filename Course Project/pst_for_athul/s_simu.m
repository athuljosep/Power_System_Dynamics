...
if pathname == 0
   ...
   len_file = length(dfile);
   % strip off .m and convert to lower case
   % dfile = lower(dfile(1:len_file-2));
...
my_waref = z1;  % wide-area PSS reference
...
pss_out = ze;
pss_open = ze;  % RTE - for open loop PSS analysis
pss_chirp = ze;  % RTE - for open loop PSS analysis
...
clear dcsp_con dcud_idx dpw_Td_idx dpw_con dpw_idx dpw_mb_idx   
...
clear ktmax l_cap l_no_cap len_file line_f line_flw line_par line_pf1 line_pf2 line_sim     
