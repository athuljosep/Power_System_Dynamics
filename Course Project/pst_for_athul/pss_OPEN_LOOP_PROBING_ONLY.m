...
% RTE - PSS script that computes the control output but doesn't send it
% anywhere. This is for analyzing open loop transfer functions.
% 
...
global  pss1 pss2 pss3 dpss1 dpss2 dpss3 pss_out pss_open pss_chirp
...
my_Tdel = 0;  % expected delay
my_dstd = 0/60;  % standard deviation of delay
my_dt = sw_con(1,7);  % time step (must be constant)
my_nsamp = ceil(sw_con(end,1)/my_dt);  % number of sample points
my_delay = ceil(my_Tdel/my_dt)*my_dt;  % desired delay
my_dsamp = my_delay/my_dt;  % number of delayed samples
my_dsamp = my_dsamp + ceil(my_dstd*randn(1,1)/my_dt);

% wide-area reference initialization

if (k - ceil(my_Tdel/my_dt) - 4/my_dt < 1)  % T/my_dt => T < 5 (or end of steady-state run)
    my_dsamp = 0;
end

if (numel(my_waref) < 1)
    my_waref = ones(1,my_nsamp);  % array initialization
end

if (n_pss ~= 0)
    n_wa = mac_int(pss_con(pss_wa_idx,2));  % set of wide-area PSS gens
    my_waref(k) = mean(mac_spd(n_wa, k-my_dsamp));  % wide-area speed reference
end

% generating the chirp signal

my_tend = sw_con(end,1);
my_chirp_T = my_tend/2;
my_chirp_f0 = 0.01;
my_chirp_f1 = 6;
my_chirp_beta = (my_chirp_f1/my_chirp_f0)^(2/my_tend);

if (my_dt*(k-1) <= my_tend/2)
    my_chirp_tt = my_dt*(k-1);
else
    my_chirp_tt = my_tend - my_dt*(k-1);
end

my_chirp_phi = 2*pi*my_chirp_f0*((my_chirp_beta^my_chirp_tt-1)/log(my_chirp_beta));
pss_chirp(:,k) = 0.25*sin(my_chirp_phi);
...
if n_pss~=0
   ...
   if flag == 0; % initialization
      if i ~= 0  % scalar computation
         ...
         pss_open(pss_exc_idx,1)=zeros(n_pss,1);
   ...
   if flag == 1 % network interface computation
      if i ~= 0 % scalar computation
         ...
         pss_out(pss_exc_idx(i),k) = pss_chirp(pss_exc_idx(i),k);
         pss_open(pss_exc_idx(i),k) = var3;
      else
         % vector computation
         if n_pss~=0
            ...
            if ~isempty(pss_wa_idx)  % wide-area PSS
               n_wa = mac_int(pss_con(pss_wa_idx,2));
               var1(pss_wa_idx) = (my_Kwa*(mac_spd(n_wa,k) - my_waref(k))...
                   + my_Kcm*(my_waref(k) - 1) - pss1(pss_wa_idx,k))./pss_con(pss_wa_idx,4);
            end
            if ~isempty(pss_p_idx)
               n_p = mac_int(pss_con(pss_p_idx,2));
         ...
         pss_out(pss_exc_idx,k) = pss_chirp(pss_exc_idx,k);
         pss_open(pss_exc_idx,k) = var3;
      end
   end
